...
type UsdtWalletTableProps = {
  items: UsdtWalletItem[];
  startIndex: number;
  onEdit: (item: UsdtWalletItem) => void;
  onDelete: (item: UsdtWalletItem) => void;
  onSetDefault: (item: UsdtWalletItem) => void;
...
export function UsdtWalletTable({
  items,
  startIndex,
  onEdit,
  onDelete,
          ...
          {items.length === 0 ? (
            <tr>
              <td colSpan={5} className="px-4 py-12 text-center text-white/60">
                Chưa có ví USDT nào. Thêm ví để nhận thanh toán đơn hàng.
              </td>
            </tr>
          ) : (
            items.map((item, index) => (
              <tr key={item.id} className="hover:bg-white/[0.03]">
                ...
                <td className="px-4 py-4">
                  ...
                  {item.label && (
                    <p className="mt-1 text-xs text-cyan-200/60">{item.label}</p>
                  )}
                </td>
                <td className="px-4 py-4 text-sm text-white/75">{item.network}</td>
                <td className="px-4 py-4">
                ...
                <td className="px-4 py-4">
                  <div className="flex justify-end gap-2">
                    ...
                    <button type="button" className={actionBtn} onClick={() => onEdit(item)} aria-label="Sửa">
