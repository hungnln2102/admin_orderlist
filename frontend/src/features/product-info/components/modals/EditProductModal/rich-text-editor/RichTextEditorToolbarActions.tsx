...
export const RichTextEditorToolbarActions: React.FC<ToolbarActionsProps> = ({
  ...
  const compactGroups: Array<{ className?: string; buttons: ActionButton[] }> = [
    {
      buttons: [
        { title: "Hoàn tác", command: "undo", icon: <ArrowUturnLeftIcon className="h-4 w-4" /> },
        { title: "Làm lại", command: "redo", icon: <ArrowUturnRightIcon className="h-4 w-4" /> },
      ],
    },
    {
      buttons: [
        { title: "Chèn liên kết", onClick: onInsertLink, icon: <LinkIcon className="h-4 w-4" /> },
        { title: "Gỡ liên kết", command: "unlink", icon: <UnlinkIcon /> },
      ],
    },
    {
  ...
  const actionGroups: ActionButton[][] = [
    [
      { title: "In đậm", command: "bold", icon: <BoldIcon className="h-4 w-4" /> },
      { title: "In nghiêng", command: "italic", icon: <ItalicIcon className="h-4 w-4" /> },
      { title: "Gạch chân", command: "underline", icon: <UnderlineIcon className="h-4 w-4" /> },
      { title: "Gạch ngang", command: "strikeThrough", icon: <StrikethroughIcon className="h-4 w-4" /> },
    ],
    [
      { title: "Danh sách không thứ tự", command: "insertUnorderedList", icon: <ListBulletIcon className="h-4 w-4" /> },
      { title: "Danh sách có thứ tự", command: "insertOrderedList", icon: <Bars3Icon className="h-4 w-4" /> },
    ],
    [
      { title: "Căn trái", command: "justifyLeft", icon: <Bars3BottomLeftIcon className="h-4 w-4" /> },
      { title: "Căn giữa", command: "justifyCenter", icon: <Bars3CenterLeftIcon className="h-4 w-4" /> },
      { title: "Căn phải", command: "justifyRight", icon: <Bars3BottomLeftIcon className="h-4 w-4 scale-x-[-1]" /> },
    ],
    [
      { title: "Trích dẫn", onClick: () => onFormatBlock("blockquote"), icon: <ChatBubbleBottomCenterTextIcon className="h-4 w-4" /> },
      { title: "Chèn đường phân cách", command: "insertHorizontalRule", icon: <MinusIcon className="h-4 w-4" /> },
    ],
  ];
