...
import type { StorefrontRenewStatusCode } from "../types/storefrontRenew.types";

export function ActivatingSpinner({
  email,
  profileName,
...
export function CheckSuccessCard({
  profileName,
...
export function OutsideOrderCard({
  outsideOrderStatus,
}: {
...
export function NeedsProfileSwitchCard({
  profileName,
  message,
}: {
  profileName: string | null;
...
export function ActivateSuccessCard({ profileName }: { profileName: string | null }) {
  return (
    <div className="rounded-2xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-5 text-center text-sm text-emerald-50">
      <div className="mb-3 flex flex-col items-center gap-3">
        <AnimatedCheckmark />
...
export function InfoCard({ message }: { message: string | null }) {
  return (
