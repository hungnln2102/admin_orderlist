...
export function SupplierOverviewCards({
  ...
  const statCards = [
    { label: "Tá»•ng Ä‘Æ¡n", value: stats?.totalOrders ?? 0 },
    { label: "ÄÃ£ thanh toÃ¡n", value: stats?.paidOrders ?? 0 },
    { label: "ChÆ°a thanh toÃ¡n", value: stats?.unpaidOrders ?? 0 },
    { label: "ÄÃ£ há»§y", value: stats?.canceledOrders ?? 0 },
  ];
    ...
    <>
      <div className="grid md:grid-cols-2 gap-3 mt-5">
        <div className="bg-white/5 rounded-2xl border border-white/5 p-5 space-y-4">
          <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-indigo-300/80">ThÃ´ng tin chung</h3>
          <div className="text-sm text-white/80 space-y-2">
            <div className="flex justify-between items-center border-b border-white/5 pb-2">
              <span className="text-white/40">TÃªn NCC</span>
              <span className="font-bold text-white">{supply?.sourceName || "--"}</span>
            </div>
            <div className="flex justify-between items-center border-b border-white/5 pb-2">
              <span className="text-white/40">NgÃ¢n hÃ ng</span>
              <span className="font-semibold text-white">{bankName}</span>
            </div>
            <div className="flex justify-between items-center border-b border-white/5 pb-2">
              <span className="text-white/40">Sá»‘ tÃ i khoáº£n</span>
              <span className="font-mono font-semibold text-white">{supply?.numberBank || "--"}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-white/40">Tráº¡ng thÃ¡i</span>
        ...
        <div className="bg-white/5 rounded-2xl border border-white/5 p-5 space-y-4">
          <h3 className="text-[11px] font-bold uppercase tracking-[0.2em] text-indigo-300/80">Tá»•ng quan thanh toÃ¡n</h3>
          <div className="grid grid-cols-2 gap-3 text-sm text-white/80">
            <div className="bg-white/5 rounded-xl p-3 border border-white/5 transition-colors hover:bg-white/10">
              <p className="text-white/50 text-[11px] font-bold uppercase tracking-wider">ÄÃ£ tráº£</p>
              <p className="text-lg font-bold text-white mt-1">{formatCurrency(stats?.totalPaidAmount || 0)}</p>
            </div>
            <div className="bg-white/5 rounded-xl p-3 border border-white/5 transition-colors hover:bg-white/10">
              <p className="text-white/50 text-[11px] font-bold uppercase tracking-wider">CÃ²n ná»£</p>
              <p className="text-lg font-bold text-orange-400 mt-1">{formatCurrency(totalUnpaid)}</p>
            </div>
            <div className="bg-white/5 rounded-xl p-3 border border-white/5 transition-colors hover:bg-white/10">
              <p className="text-white/50 text-[11px] font-bold uppercase tracking-wider">HoÃ n tiá»n</p>
              <p className="text-lg font-bold text-rose-400 mt-1">{formatCurrency(totalSupplierRefund)}</p>
            </div>
            <div className="bg-white/5 rounded-xl p-3 border border-white/5 transition-colors hover:bg-white/10">
              <p className="text-white/50 text-[11px] font-bold uppercase tracking-wider">Ná»£ Ä‘Æ¡n</p>
