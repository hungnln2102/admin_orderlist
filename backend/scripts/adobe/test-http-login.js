/**
 * Test login Adobe qua HTTP request (không dùng Puppeteer).
 *
 * Chạy: node scripts/test-http-login.js <email> <password>
 * Hoặc: node scripts/test-http-login.js  (dùng account đầu tiên từ DB)
 */

const path = require("path");
require("dotenv").config({ path: path.join(__dirname, "..", ".env") });

const { checkAccount } = require("../src/services/adobe-http");
...
(async () => {
  if (!email || !password) {
    console.log("Usage: node scripts/test-http-login.js <email> <password>");
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log("Members:", JSON.stringify(result.scrapedData?.manageTeamMembers, null, 2));
      } else {
    ...
    console.log("Test HTTP login với:", email);
