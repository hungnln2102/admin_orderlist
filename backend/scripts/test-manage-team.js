 ...
 *   EMAIL=xxx PASSWORD=yyy node scripts/test-manage-team.js
 *
 * Có thể truyền cookies đã lưu (format { cookies: [], savedAt }):
 *   COOKIES_FILE=cookies/adobe_account_1.json EMAIL=xxx PASSWORD=yyy node scripts/test-manage-team.js
 */
...
async function main() {
  ...
  if (fs.existsSync(COOKIES_FILE)) {
    try {
      ...
      savedCookiesFromDb = raw && Array.isArray(raw.cookies) ? raw : { cookies: raw.cookies || [], savedAt: raw.savedAt };
    } catch (_) {
      console.warn("[test] Không đọc được cookies file, chạy không dùng cookies.");
    }
  }

  const adobeHttp = require("../src/services/adobe-http");
