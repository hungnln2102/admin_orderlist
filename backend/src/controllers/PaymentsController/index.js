...
const listPaymentReceipts = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: "Không th? t?i biên lai thanh toán." });
  }
};
...
const confirmPaymentSupply = async (req, res) => {
  ...
  if (!Number.isInteger(parsedPaymentId) || parsedPaymentId <= 0) {
    return res.status(400).json({
      error: "ID thanh toán không h?p l?.",
    });
  }
  ...
  try {
    const updatedRow = await withTransaction(async (trx) => {
      ...
      const UNPAID_STATUS = "Chua Thanh Toán";
      const PAID_STATUS = "Ðã Thanh Toán";
      ...
      try {
        ...
        console.error(
          "[payments] Không th? d?i chi?u các don d?t hàng chua thanh toán cho hàng hóa",
          sourceId,
          orderErr
        );
      }
    ...
    if (!updatedRow) {
      return res.status(404).json({ error: "Không tìm th?y h? so thanh toán." });
    }
    res.json(updatedRow);
  } catch (error) {
    console.error(
    ...
    res.status(500).json({
      error: "Không th? xác nh?n thanh toán.",
    });
  }
};
