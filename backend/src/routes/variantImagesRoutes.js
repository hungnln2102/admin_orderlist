...
const buildVariantImageUrl = (req, filename) => {
  ...
  return `${base}/image_variant/${encodeURIComponent(filename)}`;
};

router.post("/upload", (req, res) => {
  upload.single("image")(req, res, (err) => {
    if (err) {
      logger.error("Upload variant image failed", { error: err.message });
    ...
    return res.json({
      ...
      url: buildVariantImageUrl(req, req.file.filename),
    });
  });
});
...
router.get("/", async (req, res) => {
  try {
    ...
    const items = entries
      ...
      .map((name) => ({
        fileName: name,
        url: buildVariantImageUrl(req, name),
      }));
    res.json({ items, count: items.length });
  } catch (error) {
    logger.error("List variant images failed", { error: error.message });
...
router.delete("/:fileName", async (req, res) => {
  ...
  try {
    ...
    logger.error("Delete variant image failed", { fileName, error: error.message });
