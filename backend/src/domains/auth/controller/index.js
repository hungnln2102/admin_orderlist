...
const verifyPassword = async (inputPassword, storedValue) => {
  const hashString =
    storedValue instanceof Buffer ? storedValue.toString() : String(storedValue || "");
...
const SESSION_COOKIE_MS = 1000 * 60 * 60 * 1;
...
const login = async (req, res) => {
  ...
  try {
    ...
    logger.error("[auth] Đăng nhập thất bại", {
      error: error.message,
      stack: error.stack,
...
const changePassword = async (req, res) => {
  ...
  try {
    ...
    logger.error("[auth] Thay đổi mật khẩu thất bại", {
      error: error.message,
      stack: error.stack,
...
const ensureDefaultAdmin = async () => {
  ...
  try {
    ...
    logger.error("[AUTH] Lỗi khi tạo Admin", {
      error: err.message,
      stack: err.stack,
