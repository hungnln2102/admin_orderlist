...
const listCategories = async (_req, res) => {
  try {
    const rows = await db(TABLES.category).select("*").orderBy(categoryCols.name, "asc");
    ...
    logger.error("Query failed (GET /api/categories)", {
      error: error.message,
      stack: error.stack,
    });
...
const createCategory = async (req, res) => {
  ...
  try {
    ...
    const result = await db(TABLES.category).insert(insertPayload).returning(returningCols);
    ...
    logger.error("Insert failed (POST /api/categories)", {
      error: error.message,
      stack: error.stack,
    });
...
const updateCategory = async (req, res) => {
  ...
  try {
    ...
    logger.error(`Update failed (PUT /api/categories/${id})`, {
      error: error.message,
      stack: error.stack,
    });
...
const deleteCategory = async (req, res) => {
  ...
  try {
    const deleted = await db(TABLES.category).where(categoryCols.id, id).del();
    ...
    logger.error(`Delete failed (DELETE /api/categories/${id})`, {
      error: error.message,
      stack: error.stack,
    });
