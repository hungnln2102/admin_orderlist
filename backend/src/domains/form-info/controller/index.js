...
const INPUTS_TABLE = tableName(INPUT_DEF?.tableName || "inputs", SCHEMA_INPUTS);
...
const createInput = async (req, res) => {
  try {
    ...
    const trimmedType =
      typeof type === "string" ? type.trim().toLowerCase() : "text";
...
const createForm = async (req, res) => {
  try {
    ...
    res.status(201).json({
      ...
      description:
        formRow[fCols.description] ?? formRow.description ?? (trimmedDesc || null),
      inputIds: ids,
