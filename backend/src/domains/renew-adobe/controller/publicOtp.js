...
const publicGetOtp = async (req, res) => {
  ...
  try {
    ...
    const trackRow = await db(TRACK_TABLE)
      .select(TRACK_COLS.OTP_SOURCE, TRACK_COLS.SYSTEM_NOTE, TRACK_COLS.YUNA_ORDER_CODE)
    ...
    const accountRow = await db(ACCOUNT_TABLE)
      .select(ACCOUNT_COLS.MAIL_BACKUP_ID, ACCOUNT_COLS.YUNA_ORDER_CODE)
    ...
    const yunaOrderCode = trackRow?.[TRACK_COLS.YUNA_ORDER_CODE] || accountRow?.[ACCOUNT_COLS.YUNA_ORDER_CODE] || null;
