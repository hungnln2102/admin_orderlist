 ...
 * - Sequence `payment_amount_suffix_seq` (1..100, CYCLE) cấp suffix luân phiên.
 * - Bảng `orders.order_payment_slots` lưu lifecycle từng chu kỳ thanh toán
 *   (đơn mới + mỗi lần gia hạn = 1 slot). Webhook resolve order theo
 *   `(receiver_account, expected_amount)` thay vì transaction code.
 *
 * SQL đồng bộ: database/migrations/107_order_payment_slots.sql
 */
...
-- Đảm bảo mỗi (receiver, expected_amount) chỉ có duy nhất 1 slot pending → tránh trùng suffix.
...
-- Webhook resolve order qua amount: hot path.
...
-- Lấy slot pending mới nhất theo đơn (UI / runRenewal).
...
-- Monitoring usage suffix theo (receiver, base_amount).
...
exports.down = async function down() {
  // Slot lifecycle đã đan vào order resolution của webhook — không tự gỡ an toàn.
};
