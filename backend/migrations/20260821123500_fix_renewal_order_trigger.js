...
exports.up = async function(knex) {
  await knex.raw(`
    ...
    DECLARE
      ...
      v_refund numeric := 0;
      v_latest_id bigint;
      v_latest_logged_at timestamptz;
      v_has_recent_log boolean := false;
    ...
    BEGIN
      ...
      IF TG_OP = 'UPDATE' THEN
        SELECT id, logged_at INTO v_latest_id, v_latest_logged_at
        FROM business.supplier_order_cost_log
        WHERE order_list_id = NEW.id
        ORDER BY id DESC
        LIMIT 1;

        IF v_latest_logged_at IS NOT NULL AND v_latest_logged_at >= NOW() - INTERVAL '15 days' THEN
          v_has_recent_log := true;
        END IF;
        ...
        IF (
          ...
          OR (
            COALESCE(OLD.status, '') IN (v_unpaid, v_renewal)
            AND NEW.status IS NOT DISTINCT FROM v_processing
          )
          OR (
            COALESCE(OLD.status, '') = v_processing
            AND NEW.status IS NOT DISTINCT FROM v_paid
            AND NOT v_has_recent_log
          )
          OR (
            (NEW.status IS NOT DISTINCT FROM v_paid OR NEW.status IS NOT DISTINCT FROM v_processing)
            AND v_latest_id IS NULL
            AND NEW.supply_id IS NOT NULL
          )
        ) THEN
          INSERT INTO business.supplier_order_cost_log (
            order_list_id,
    ...
    BEGIN
      ...
      IF TG_OP = 'UPDATE' THEN
        ...
        IF (
          ...
          -- ADDED: Support post-facto supplier assignment on processing/paid orders that don't have a log entry yet
          OR (
            (NEW.status IS NOT DISTINCT FROM v_paid OR NEW.status IS NOT DISTINCT FROM v_processing)
            AND v_latest_id IS NULL
            AND NEW.supply_id IS NOT NULL
