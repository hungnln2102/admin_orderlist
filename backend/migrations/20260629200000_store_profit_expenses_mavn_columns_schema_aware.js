...
exports.up = async function up(knex) {
  ...
  await knex.raw(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS uq_store_profit_expenses_mavn_order
      ON "${fin}".com_profit_expenses (linked_order_code)
...
exports.down = async function down(knex) {
  ...
  await knex.raw(`
    ...
    ALTER TABLE "${fin}".com_profit_expenses DROP COLUMN IF EXISTS expense_meta;
    ALTER TABLE "${fin}".com_profit_expenses DROP COLUMN IF EXISTS linked_order_code;
    ALTER TABLE "${fin}".com_profit_expenses DROP CONSTRAINT IF EXISTS store_profit_expenses_expense_type_check;
    ALTER TABLE "${fin}".com_profit_expenses ADD CONSTRAINT store_profit_expenses_expense_type_check
