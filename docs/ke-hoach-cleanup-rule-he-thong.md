...
- [x] Đã bỏ khỏi `dbSchema` các definition không còn tồn tại trong DB local: `common.status`, `system_automation.order_auto_keys`, `receipt.refund`, `partner.supplier.account_holder`, `wallet.wallet_transactions.promo_code`, `customer_web.accounts.mail_backup_id`, `dashboard.com_profit_expenses.linked_order_code`, `dashboard.com_profit_expenses.expense_meta`.
...
| `src/features/product-price/constants.ts` | Chuỗi hiển thị domain (`mavrykpremium.com`). |
