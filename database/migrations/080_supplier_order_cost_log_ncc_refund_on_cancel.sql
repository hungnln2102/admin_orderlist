-- Ghi đúng tiền hoàn NCC (prorata theo vốn / ngày) khi hủy đơn, thay vì để v_refund_for_log = 0.
-- Trùng với calcRemainingImport (Phoenix): cost × (canceled → expiry) / days.
-- (order_list.refund = giá bán prorata — không dùng cho cột log NCC.)
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- Xóa / archive: ghi dòng mới; tiền hoàn = prorata NCC theo vốn (không theo cột order_list.refund).
    IF (
