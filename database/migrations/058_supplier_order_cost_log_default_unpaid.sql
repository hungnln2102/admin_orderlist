-- 1) Đưa các dòng log hiện có về trạng thái TT NCC «Chưa Thanh Toán».
-- 2) Trigger chỉ tạo/cập nhật chi phí; TT NCC mặc định luôn «Chưa Thanh Toán».
--    Đánh dấu «Đã Thanh Toán» (NCC) chỉ qua API xác nhận thanh toán NCC (PaymentsController).

UPDATE partner.supplier_order_cost_log
SET ncc_payment_status = 'Chưa Thanh Toán',
    logged_at = NOW()
WHERE TRIM(COALESCE(ncc_payment_status::text, '')) = 'Đã Thanh Toán';
