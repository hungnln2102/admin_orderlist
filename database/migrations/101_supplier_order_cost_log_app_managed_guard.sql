-- =============================================================================
-- Guard: app-managed supplier change (Flow A / Flow B đổi NCC)
-- =============================================================================
-- Khi service `domains/supplier-change` xử lý đổi NCC, nó SET LOCAL
-- `app.supplier_change_managed = 'on'` trong transaction. Trigger phải
-- bỏ qua HOÀN TOÀN nhánh sync supply_id/cost/refund để service tự quản lý:
--   - Flow A (≤5 ngày): cập nhật cost trên đơn + UPDATE log mới nhất bằng code.
--   - Flow B Chưa TT: xóa log cũ + INSERT log NCC mới (skip nếu NCC mới = Mavryk).
--   - Flow B Đã TT: KHÔNG xóa log cũ; INSERT log hoàn (supply_id cũ, refund_amount=R)
--     + INSERT log NCC mới (skip nếu Mavryk).
--
-- Vẫn giữ các nhánh khác (status transitions: Chưa TT → Đã TT, Đã TT → Hoàn/Hủy,
-- INSERT MAVN/MAVT khi tạo đơn) như cũ trong migration 091.
-- =============================================================================
...
BEGIN
  ...
  -- Service `supplier-change` đã set GUC; toàn bộ logic ghi/xóa/sửa log do service tự lo.
  ...
  -- Khi app quản lý (đổi NCC), KHÔNG động vào log (kể cả nhánh xóa Mavryk).
