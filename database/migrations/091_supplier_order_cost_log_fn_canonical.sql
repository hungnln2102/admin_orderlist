-- =============================================================================
-- CANONICAL: partner.fn_supplier_order_cost_log_on_success
-- =============================================================================
-- Trigger trên orders.order_list. `dashboard_monthly_summary.total_import` + `total_profit`
-- (theo đơn) **chỉ** do partner.fn_recalc_dashboard_total_import khi supplier_order_cost_log đổi.
--
-- Mốc chính ghi log cho đơn thường (non-Mavryk): **chuyển sang Đã Thanh Toán** (Chưa TT /
-- Cần Gia Hạn / ĐXL chưa có log → Đã TT). MAVN/MAVT: INSERT khi tạo đơn đã Đã TT.
-- Đã TT → hoàn/hủy: thêm dòng log NCC. Chỉnh cost/supply khi ĐXL/Đã TT: UPDATE dòng log (vẫn kích hoạt recalc).
--
-- 1) supply_id NULL → bỏ qua.
-- 2) NCC Mavryk: xóa log đơn (đơn thường không theo dòng NCC này).
-- 3) cost/refund/days: regexp_replace an toàn trước khi numeric.
-- =============================================================================
